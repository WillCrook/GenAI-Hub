/* Data controller for the Learn AI Resource Library page. */
(function (window) {
    'use strict';

    var hub = window.GenAIHub || {};
    hub.controllers = hub.controllers || {};
    if (hub.controllers.learnAiLibrary) return;
    ...
    function selectResources(resources) {
        return resources.filter(function (resource) {
            return resource.published === true && resource.librarySection === 'learn-ai';
        });
    }
    ...
    hub.controllers.learnAiLibrary = controller;
    window.GenAIHub = hub;
    controller.ready = load();
    controller.ready.catch(function (error) {
        if (window.console && console.error) console.error('Learn AI Library resource loading failed.', error);
    });
})(window);
