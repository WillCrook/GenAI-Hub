/* Data controller for featured resources on the Community page. */
(function (window) {
    'use strict';

    var hub = window.GenAIHub || {};
    hub.controllers = hub.controllers || {};
    if (hub.controllers.communityFeatured) return;
    ...
    function selectResources(resources) {
        return resources.filter(function (resource) {
            return resource.published === true && resource.featured === true && resource.librarySection === 'community';
        });
    }
    ...
    hub.controllers.communityFeatured = controller;
    window.GenAIHub = hub;
    controller.ready = load();
    controller.ready.catch(function (error) {
        if (window.console && console.error) console.error('Community featured resource loading failed.', error);
    });
})(window);
