/* Data controller for the unified Community Resource Library page. */
(function (window) {
    'use strict';

    var hub = window.GenAIHub || {};
    hub.controllers = hub.controllers || {};
    if (hub.controllers.communityLibrary) return;

    var state = { status: 'idle', resources: [], error: null };
    var request = null;

    function selectResources(resources) {
        return resources.filter(function (resource) { return resource.published === true; });
    }
    ...
    hub.controllers.communityLibrary = controller;
    window.GenAIHub = hub;
    controller.ready = load();
    controller.ready.catch(function (error) {
        if (window.console && console.error) console.error('Community Library resource loading failed.', error);
    });
})(window);
