/* Data controller for featured resources on the Challenges page. */
(function (window) {
    'use strict';

    var hub = window.GenAIHub || {};
    hub.controllers = hub.controllers || {};
    if (hub.controllers.challengesFeatured) return;
    ...
    function selectResources(resources) {
        return resources.filter(function (resource) {
            return resource.published === true && resource.featured === true && resource.librarySection === 'challenges';
        });
    }
    ...
    hub.controllers.challengesFeatured = controller;
    window.GenAIHub = hub;
    controller.ready = load();
    controller.ready.catch(function (error) {
        if (window.console && console.error) console.error('Challenges featured resource loading failed.', error);
    });
})(window);
