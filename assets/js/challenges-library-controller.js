/* Data controller for the Challenges Resource Library page. */
(function (window) {
    'use strict';

    var hub = window.GenAIHub || {};
    hub.controllers = hub.controllers || {};
    if (hub.controllers.challengesLibrary) return;
    ...
    function selectResources(resources) {
        return resources.filter(function (resource) {
            return resource.published === true && resource.librarySection === 'challenges';
        });
    }
    ...
    hub.controllers.challengesLibrary = controller;
    window.GenAIHub = hub;
    controller.ready = load();
    controller.ready.catch(function (error) {
        if (window.console && console.error) console.error('Challenges Library resource loading failed.', error);
    });
})(window);
