(function () {
  ...
  var SHARED = ['id','title','summary','author_name','author_organisation','author_course','author_year_of_study','date_published','date_updated','library_section','skill_areas','tags','featured','main_carousel','published','estimated_minutes','thumbnail_src','thumbnail_alt'];
  var DEFINITIONS = [
  ...
  function validateRow(row,definition,separator,id) {
    ...
    ['featured','main_carousel','published'].forEach(function (field) { if (parseBoolean(v[field])===null) addIssue('error',row.sheet,row.number,field,id,'Enter TRUE or FALSE.'); });
  ...
  var COLUMN_BY_PATH = {
    ...
    librarySection:'library_section',skillAreas:'skill_areas',tags:'tags',featured:'featured',mainCarousel:'main_carousel',published:'published',
  ...
  function compareUpdatedResources(left,right){var byUpdated=right.dateUpdated.localeCompare(left.dateUpdated);if(byUpdated)return byUpdated;var byPublished=right.datePublished.localeCompare(left.datePublished);if(byPublished)return byPublished;return left.id.localeCompare(right.id,'en-GB',{sensitivity:'base'});}
